<script lang="ts">
    import { Component, Mixins } from "vue-property-decorator"
    import KeywordElement from '../KeywordElement.vue'

    @Component({
        components: {
            KeywordElement,
        }
    })
    export default class OpenAvailableBrowser extends Mixins(KeywordElement) {
        mounted() {
            this.init();
        }

        init() {
            var keys = Object.keys(this.value.property)
            if (!keys || keys.length < 1) {
                this.value.property = new Map()
                this.$set(this.value.property, "url", "None")
                this.$set(this.value.property, "headless", null)
                this.$set(this.value.property, "maximized", null)
            }

            if (!this.value.property.hasOwnProperty("url")) {
                this.$set(this.value.property, "url", "None")
            }
        }
    }
</script>