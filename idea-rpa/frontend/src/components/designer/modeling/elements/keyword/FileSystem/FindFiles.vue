<script lang="ts">
    import { Component, Mixins } from "vue-property-decorator"
    import KeywordElement from '../KeywordElement.vue'

    @Component({
        components: {
            KeywordElement,
        }
    })
    export default class EmptyDirectory extends Mixins(KeywordElement) {
        mounted() {
            this.init();
        }

        init() {
            var keys = Object.keys(this.value.property)
            if (!keys || keys.length < 1) {
                this.value.property = new Map()
                this.$set(this.value.property, "pattern", null)
                this.$set(this.value.property, "include_dirs", true)
                this.$set(this.value.property, "include_files", true)
                this.$set(this.value.property, "returnVal", [{
                    defaultValue: "matches"
                }])
            }

            if (!this.value.property.hasOwnProperty("include_dirs")) {
                this.$set(this.value.property, "include_dirs", true)
            }
            if (!this.value.property.hasOwnProperty("include_files")) {
                this.$set(this.value.property, "include_files", true)
            }
            if (!this.value.property.hasOwnProperty("returnVal")) {
                this.$set(this.value.property, "returnVal", [{
                    defaultValue: "matches"
                }])
            }
        }
    }
</script>